...
def get_signal(df):
    ...
    cross_buy = (ema_fast > ema_slow) and rsi_buy and adx_strong and ema_valid
    cross_sell = (ema_fast < ema_slow) and rsi_sel and adx_strong and ema_valid
...
def get_confirm(df, direction):
    ...
    buy_candle = (candle_low > ema_trend)
    sell_candle = (candle_high < ema_trend)
    rsi = calculate_rsi(close, RSI_CONFIRM_PERIOD)

    curr_rsi = rsi.iloc[-2]
    real_rsi = rsi.iloc[-1]

    rsi_buy = 30 < curr_rsi < 55 and real_rsi > curr_rsi
    rsi_sel = 45 < curr_rsi < 70 and real_rsi < curr_rsi
    ...
    cross_buy = (ema_fast > ema_slow and ema_slow > ema_trend) and buy_candle and rsi_buy and adx_strong and strong_candle_buy
    cross_sell = (ema_fast < ema_slow and ema_slow < ema_trend) and sell_candle and rsi_sel and adx_strong and strong_candle_sell
