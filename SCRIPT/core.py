...
ADX_PERIOD = 8        # default 5 (ubah kalau mau)
...
def sma_cross_signals(df):
    ...
    # Hitung SMA 30 & SMA 60
    df['SMA30'] = df['close'].rolling(window=30).mean()
    df['SMA60'] = df['close'].rolling(window=60).mean()

    # Buat column selisih
    df['prev_diff'] = df['SMA30'].shift(10) - df['SMA60'].shift(10)
    df['curr_diff'] = df['SMA30'] - df['SMA60']

    # Cross Up (BUY) → dulu di bawah, sekarang di atas
    df['BUY'] = (df['prev_diff'] < 0) & (df['curr_diff'] > 0)

    # Cross Down (SELL) → dulu di atas, sekarang di bawah
    df['SELL'] = (df['prev_diff'] > 0) & (df['curr_diff'] < 0)
    ...
    while True:
        try:
            ...
            if not hold_buy_signal and not hold_buy_signal:
                print(f"{datetime.now()} | Price: {price} | RSI: {np.nan if np.isnan(rsi) else round(rsi, 2)} | ATR: {round(atr,5) if not pd.isna(atr) else 'NA'} | EMA BUY: {df['BUY'].iloc[-1]} | EMA SELL: {df['SELL'].iloc[-1]}")
            ...
            if prev_rsi is not None:
                ...
                if hold_buy_signal and (prev_rsi < RSI_OS and rsi > RSI_OS) and adx > 25:
                    ...
                    hold_buy_signal = False  # Release after entry
                ...
                if hold_sell_signal and (prev_rsi > RSI_OB and rsi < RSI_OB) and adx > 25:
                    ...
                    hold_sell_signal = False  # Release after entry
