...
def stochastic_signal(df):
    ...
    buy_stoch = (k_prev < STOCH_LOW_LEVEL) and (d_prev < STOCH_LOW_LEVEL) and (k_now > STOCH_LOW_LEVEL) and (d_now > STOCH_LOW_LEVEL)
    sell_stoch = (k_prev > STOCH_HIGH_LEVEL) and (d_prev > STOCH_HIGH_LEVEL) and (k_now < STOCH_HIGH_LEVEL) and (d_now < STOCH_HIGH_LEVEL)
...
def main():
    ...
    while True:
        try:
            ...
            # VALIDASI SIGNAL
            if signal_tren == 'BUY' and signal_stoch == 'BUY':
                buy_signal = True
                sell_signal = False
            if signal_tren == 'SELL' and signal_stoch == 'SELL':
                sell_signal = True
                buy_signal = False
            ...
            if num_position < MAX_POSITIONS:
                logger.info("PAIR : %s | TREND : %s | STOCH CROSS : %s",SYMBOL, signal_tren, signal_stoch)
            else:
